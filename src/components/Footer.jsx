...
function Footer() {
  return (
    <div data-scroll data-scroll-section data-scroll-speed='.2' className='w-full h-screen bg-[#CDEA68] px-12 py-8 flex gap-5'>
      <div className="w-1/2 h-full flex flex-col justify-between">
        <h1 className='uppercase text-[9vw] font-["Founders"] leading-[0.7] mt-6'>eye-<br />opening</h1>
      ...
      <div className="w-1/2 h-full flex flex-col justify-between">
        <h1 className='uppercase text-[9vw] font-["Founders"] leading-[0.7] mt-6 -mb-4'>presentations</h1>
        <div className="mid flex justify-between items-center w-full h-[65%]">
          <div className="mid-left flex flex-col gap-10 w-1/2 h-full">
            <div className="S h-1/3 w-full font-['NeueMontreal'] text-md">
              <h2 className='uppercase mb-2'>S:</h2>
              {['instagram','behance','facebook','linkedin'].map((elem,idx) => {
                return <a key={idx} className='capitalize block'>{elem}</a>
              })}
            </div>
            <div className="L h-1/3 w-full font-['NeueMontreal'] text-md">
              <h2 className='uppercase mb-2'>L:</h2>
              {['202-1965 W 4th Ave','Vancouver, Canada','30 Chukarina St','Lviv, Ukraine'].map((elem,idx) => {
                return <a key={idx} className='capitalize block'>{elem}</a>
              })}
            </div>
            <div className="E h-1/3 w-full font-['NeueMontreal'] text-md">
              <h2 className='uppercase mb-2'>E:</h2>
              <a className=''>hello@ochi.design</a>
            </div>
          </div>
          <div className="mid-right h-full w-1/2 flex items-center justify-end">
            <div className="M h-1/4 w-full font-['NeueMontreal'] text-md ml-40">
              <h2 className='uppercase mb-2'>M:</h2>
              {['home','services','our work','about us','insights','contact us'].map((elem,idx) => {
                return <a key={idx} className='capitalize block'>{elem}</a>
              })}
            </div>
          </div>
        </div>
        <div className="foot w-full flex justify-between text-sm text-zinc-600">
          <a href="#">© ochi design 2025. Legal Terms</a>
